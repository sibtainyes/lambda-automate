...
def lambda_handler(event, context):
    ...
    if len(devices_list) < 1 and group_id != "":
        ...
        for device in devices_list:
            ...
            update_expression = 'SET group_id = :group_id'
        ...
        if len(failed_to_add) > 0:
            data = {"failed to add": failed_to_add}

            return response.get_response(code=Constants.BAD_REQUEST, message="failed to add some devices", data=data)
